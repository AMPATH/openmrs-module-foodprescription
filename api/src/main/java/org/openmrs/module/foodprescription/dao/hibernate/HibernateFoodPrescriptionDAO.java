...
public class HibernateFoodPrescriptionDAO implements FoodPrescriptionDAO {
	...
	public List<FoodEncounter> getFoodEncounterEnteredInLastFewDays(Integer daysEntered) {
		...
		Criteria crit = sessionFactory.getCurrentSession().createCriteria(FoodEncounter.class).add(Restrictions.gt("dateCreated", daysAgo.getTime())).addOrder(Order.desc("creator"));
		@SuppressWarnings("unchecked")
